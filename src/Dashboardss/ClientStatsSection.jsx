...
const ClientStatsSection = ({
    ...
    if (!mostActiveClients.length && !topServicesRequested.length && !lastLoginTimes.length && !allUsers.length) {
                    ...
                    <ul>
                        {mostActiveClients.map((client, index) => (
                            <li key={`active-client-${client.id || index}`}>
                                <strong>{client.username}</strong>: {client.total_bookings} bookings
                            </li>
                        ))}
                    </ul>
                ) : (
                    ...
                    <ul>
                        {topServicesRequested.map((service, index) => (
                            <li key={`service-request-${service.service_type || index}`}>
                                <strong>{service.service_type}</strong>: {service.request_count} requests
                            </li>
                        ))}
                    </ul>
                ) : (
